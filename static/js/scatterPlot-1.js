// scatterPlot.js
// corruption_index_score at X-axis
// other variables at the Y-axis
...
function createScatterPlot(data, xKey, yKey, topN = 'all') {
  ...
  const layout = {
    title: 'Correlation between ' + xKey + ' and ' + yKey,
    xaxis: { title: xKey },
    yaxis: { title: yKey },
    margin: { t: 30 }
  };
...
function loadAndCreateScatterPlot() {
  fetch(dataFilePath)
    ...
    .then(jsonData => {
      ...
      createScatterPlot(window.povertyData, 'corruption_index_score', 'female_gross_enrolment');
    })
...
function handleTopNChange(event) {
  ...
  let xKey = 'corruption_index_score', yKey;
  switch (selectedDataset) {
    case 'female_gross_enrolment':
      yKey = 'female_gross_enrolment';
      break;
